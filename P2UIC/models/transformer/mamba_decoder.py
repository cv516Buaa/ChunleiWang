...
from models.transformer.mamba_encoder import MutiHeadSelfAttention
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #output_0 = output_0 + self.mlp_channels(self.norm2(output_0))
        #output_1 = output_1 + self.mlp_channels(self.norm2(output_1))
    ...
    def step(self, x, cache):
        ...
        output = output + x
        # output = output + self.mlp_channels(output)
        output = output + self.mlp_channels(self.norm2(output))
        return output, cache
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        # self.GLU = nn.Sequential(nn.Linear(2 * config.d_model, 4 * config.d_model), nn.GLU())
    ...
    def selective_scan(self, x, delta, A, B, C, D, isdecoder=False):
        ...
        # Similarity = torch.matmul(mean, x.permute(0, 2, 1))
        #
        # Similarity = torch.softmax(Similarity, -1)
        #
        # sorted_values, sorted_indices = torch.sort(Similarity[:, 0, :], dim=-1)
        # # indices = sorted_indices.unsqueeze(1).expand(-1, out.size(-1), -1).permute(0, 2, 1)
        # indices = sorted_indices.unsqueeze(-1).expand(-1, -1, x.size(-1))
        # original_indices = torch.argsort(indices, dim=1)
        # x = torch.gather(x, 1, indices)

        deltaA = torch.exp(delta.unsqueeze(-1) * A)  # (B, L, ED, N) 50,49,1024,16
        deltaB = delta.unsqueeze(-1) * B.unsqueeze(2)  # (B, L, ED, N) 50,49,1024,16

        BX = deltaB * (x.unsqueeze(-1))  # (B, L, ED, N) 50,49,1024,16

        hs = pscan(deltaA, BX)  # 50,49,1024,16
        ...
        # y = torch.gather(y, 1, original_indices)
        return y
