...
class ImageDetectionsField(RawField):
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        #precomp_data = np.load(self.detections_path + '/%s.npy' % image_name)[1:,]  # 50,768 remove cls
        # clip feature
        precomp_data = np.load(self.detections_path + '/%s.npy' % image_name)
        precomp_data = precomp_data[0]
        precomp_data = np.expand_dims(precomp_data, axis=0)
        precomp_data = precomp_data.repeat([49], axis=0)
