...
class Dataset(object):
    ...
    def __getitem__(self, i):
        ...
        for field_name, field in self.fields.items():
            ...
            #     with open('test_sydney_xe', 'a') as f:
            #         f.write(filename + ',')
            #     f.close()
            # ##########################################
            # if tem.split('.')[-1] == 'tif':
...
class Sydney(PairedDataset):
    ...
    @classmethod
    def get_samples(cls, img_root, ann_root, id_root,ids_dataset=None):
        train_samples = []
        val_samples = []
        test_samples = []
    ...
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        with nostdout():  # dataset处理后，每个example自带编号(每个数据集都从00000开始)，都是image绝对路径+一个caption
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(img_root, ann_root,
                                                                                          id_root)
        examples = self.train_examples + self.val_examples + self.test_examples  # 三个数据集合在一起
        super(UIC, self).__init__(examples, {'image': image_field, 'text': text_field})  # 调用PairedDataset
    ...
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        with nostdout():  # dataset处理后，每个example自带编号(每个数据集都从00000开始)，都是image绝对路径+一个caption
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(img_root, ann_root,
                                                                                          id_root)
        examples = self.train_examples + self.val_examples + self.test_examples  # 三个数据集合在一起
        super(SUIM_IC, self).__init__(examples, {'image': image_field, 'text': text_field})  # 调用PairedDataset
    ...
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        with nostdout():  # dataset处理后，每个example自带编号(每个数据集都从00000开始)，都是image绝对路径+一个caption
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(img_root, ann_root,
                                                                                          id_root)
        examples = self.train_examples + self.val_examples + self.test_examples  # 三个数据集合在一起
        super(UWSeg_IC, self).__init__(examples, {'image': image_field, 'text': text_field})  # 调用PairedDataset
...
class UCM(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        with nostdout():  # dataset处理后，每个example自带编号(每个数据集都从00000开始)，都是image绝对路径+一个caption
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(img_root, ann_root, id_root)
        examples = self.train_examples + self.val_examples + self.test_examples  # 三个数据集合在一起
...
class NWPU(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        roots['train'] = {
            'img': os.path.join(img_root, ''),
            'cap': os.path.join(ann_root, 'dataset_nwpu.json')
        }
        roots['val'] = {
            'img': os.path.join(img_root, ''),
            'cap': os.path.join(ann_root, 'dataset_nwpu.json')
        ...
        imgs = json.load(open(os.path.join(ann_root, 'dataset_nwpu.json'), 'r'))


        for i in imgs:
            with nostdout():  # dataset处理后，每个example自带编号(每个数据集都从00000开始)，都是image绝对路径+一个caption# i is airplane, airport...
                self.train_examples_tem, self.val_examples_tem, self.test_examples_tem = self.get_samples(img_root, ann_root, ids,i)
            if i=='airplane':
                self.train_examples = self.train_examples_tem
                self.val_examples = self.val_examples_tem
                self.test_examples = self.test_examples_tem
            else:
                self.train_examples = self.train_examples + self.train_examples_tem
                self.val_examples = self.val_examples + self.val_examples_tem
                self.test_examples = self.test_examples + self.test_examples_tem
    ...
    @classmethod
    def get_samples(cls, img_root, ann_root, ids_dataset=None,cs=None):
        train_samples = []
        val_samples = []
        test_samples = []

        dataset = json.load(open(os.path.join(ann_root, 'dataset_nwpu.json'), 'r'))[cs]
